...
export const getHeroNamesById = function (heroId) {
    for (let i = 0; i < heroesList.length; i++) {
        if (heroesList[i].id === heroId) {
            return {
                name: heroesList[i].name,
                nameForUrl: heroesList[i].name.slice(14),
                localized_name: _.capitalize((heroesList[i].name.slice(14).replace('_', ' ')))
